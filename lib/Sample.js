...
Sample.prototype = new Resource();

Sample.prototype.parent = Resource.prototype;

Sample.prototype.create = function (dataset, args, retry, cb) {
  /**
   * Creates a sample and builds customized error and resource info
   *
  ...
  var options,
    message = 'Failed to create the sample. First parameter must be' +
              ' a dataset.',
  ...
  self.parent.create.call(self, constants.SAMPLE, [constants.DATASET], message,
                          resourceId.resource,
                          options.args, options.retry, options.cb);

};

Sample.prototype.list = function (query, cb) {
  this.parent.list.call(this, constants.SAMPLE, query, cb);
};

Sample.prototype.createAndWait = function (dataset, args, retry, cb) {
  var self = this;
