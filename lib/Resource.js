...
Resource.prototype.get = function (resource, finished, query, cb) {
  ...
  if (params.finished) {
    ...
    if ((typeof params.finished) === 'boolean') {
      params.finished = {retries: Infinity, wait: 5000};
    }
    finishObject = this;
  }
  ...
  this.connection.request(reqOptions,
      ...
      if (code === constants.HTTP_OK) {
        ...
        if (params.finished) {
          ...
          if (params.finished.retries > 0) {
            params.finished.retries -= 1;
            setTimeout(function () {finishObject.get(params.resource,
                                                     params.finished,
                                                     params.query,
                                                     params.cb); },
                                    params.finished.wait);
          } else {
            return params.cb(new Error("Retries limit exceeded"), result);
          }
          return;
        }
        logger.debug(result);
...
Resource.prototype.create = function (type, origins, message,
  ...
  this.get(resourceId.resource, options.finished, sendRequest);
};
