...
function LocalSupervised(model, connection) {
  ...
  if ((typeof model) === 'string' ||
      utils.getStatus(model).code !== constants.FINISHED) {
    try {
      self.resourceId = utils.getResource(model);
    } catch (err) {
    ...
    if ((typeof self.resourceId) === 'undefined') {
      ...
      utils.tryStoredFile(model, self.resourceId.type, fillStructure);
    } else {
      ...
      if (filename != null) {
        return new LocalSupervised(filename);
      }
...
LocalSupervised.prototype.predictProbability = function (
  ...
  if (this.ready) {
    switch (this.resourceId.type) {
      case "model":
        predictArguments = [arguments[0], undefined, arguments[1]];
        break;
      case "ensemble":
        predictArguments = [arguments[0], undefined, arguments[1]];
        break;
    ...
    return this.localModel.predictProbability.apply(self.localModel,
                                                    predictArguments);
  } else {
    this.on('ready',
...
LocalSupervised.prototype.predict = function (
  inputData, addUnusedFields, operatingPoint, cb) {
  /**
   * Combined prediction for input data
   *
   * @param {object} inputData Input data to predict
   * @param {boolean} addUnusedFields Adding the fields in input data not used
  ...
  if (this.ready) {
    switch (this.resourceId.type) {
      case "model":
        predictArguments = [arguments[0], undefined, undefined, arguments[1],
                            arguments[2], undefined, arguments[3]];
        break;
      case "ensemble":
        predictArguments = [arguments[0], undefined,
                            {addUnusedFields: arguments[1],
                             operatingPoint: arguments[2]},
                            arguments[3]];
        break;
      default:
        predictArguments = arguments;
