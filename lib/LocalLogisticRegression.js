...
LocalLogisticRegression.prototype.formatFieldCodings = function () {
  ...
  var fieldCodings = JSON.parse(JSON.stringify(this.fieldCodings)),
    coding, coefficients, index, added, index2, expectedLength, index3;
  if (fieldCodings.constructor === Array) {
    ...
    for (index = 0; index < fieldCodings.length; index++) {
      if (typeof fieldCodings[index].coefficients === 'undefined') {
        ...
        expectedLength = this.categories[fieldCodings[index].field].length + 1;
        for (index2 = 0; index2 < fieldCodings[index].coefficients.length; index2++) {
          if (fieldCodings[index].coefficients[index2].length < expectedLength) {
            added = expectedLength - fieldCodings[index].coefficients[index2].length;
            for (index3 = 0; index3 < added; index3++) {
              fieldCodings[index].coefficients[index2].push(0);
            }
          }
        }
      }
      ...
      this.fieldCodings[fieldCodings[index].field][coding] = coefficients;
    }
  }
};
