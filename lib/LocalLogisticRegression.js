...
LocalLogisticRegression.prototype.logisticPredict = function (inputData) {
  ...
  var uniqueTerms, probabilities = [], total = 0, index = 0, index2 = 0, len, probability,
    category, coefficients, probabilities_sorted = [];
  ...
  for (category in this.coefficients) {
    if (this.coefficients.hasOwnProperty(category)){
      probabilities[index2].probability /= total;
    }
    index2++;
  }
  probabilities_sorted = probabilities_sorted.concat(probabilities);
  probabilities_sorted.sort(function(a, b) {
      a = a.probability;
      b = b.probability;
      return a > b ? -1 : (a < b ? 1 : 0);
  });
  return {
    prediction: probabilities_sorted[0].category,
    probability: probabilities_sorted[0].probability,
...
LocalLogisticRegression.prototype.mapCoefficients = function () {
  ...
  for (index = 0; index < len; index++) {
    ...
    if (EXPANSION_ATTRIBUTES.hasOwnProperty(optype)) {
      ...
      // coefficient
      length =
        this.fields[fieldId].summary[EXPANSION_ATTRIBUTES[optype]].length + 1;
    } else {
