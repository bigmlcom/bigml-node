    ...
    if ((typeof self.resourceId) === 'undefined') {
      ...
      try {
        fs.readFile(resource, 'utf8', function (err, data) {
          if (err) {
            throw new Error('Failed to read local logistic file: ' + resource);
          }
          try {
            resource = JSON.parse(data);
            fillStructure(null, resource);
          } catch (jsonErr) {
            throw new Error('Failed to parse the JSON logistic in: ' + resource);
          }
        });
      } catch (errf) {
        // if no file is read, throw error reading file
        throw new Error('Cannot build a Logistic Regression from' +
                        ' this resource: ' +
                        resource);
      }
    } else {
      ...
      if (typeof connection !== 'undefined' &&
        ...
        try {
          ...
          if (fs.existsSync(filename)) {
            return new LocalLogisticRegression(filename);
          }
        } catch (err) {}
      }
