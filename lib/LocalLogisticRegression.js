          ...
          for (field in fields) {
            if (fields.hasOwnProperty(field)) {
              ...
              if (fieldInfo.optype == 'text') {
                self.term_forms[field] = {}
                mapInfo = fieldInfo.summary['term_forms'];
                len = mapInfo.length;
                for (index = 0; index < len; index++) {
                  self.term_forms[mapInfo[index][0]] =
                    self.term_forms[mapInfo[index][1]];
                }
                self.tag_clouds[field] = [];
                mapInfo = fieldInfo.summary['tag_cloud'];
                len = mapInfo.length;
                for (index = 0; index < len; index++) {
                  self.tag_clouds[field].push(mapInfo[index][0]);
                }
                self.term_analysis[field] = {}
                mapInfo = fieldInfo.summary['term_analysis'];
                len = mapInfo.length;
...
LocalLogisticRegression.prototype.mapCoefficients = function () {
  ...
  for (index = 0; index < len; index++) {
    ...
    if (EXPANSION_ATTRIBUTES.hasOwnProperty(optype)) {
      ...
      // coefficient
      length = len(
        this.fields[fieldId].summary[EXPANSION_ATTRIBUTES[optype]]);
...
LocalLogisticRegression.prototype.categoryProbability = function (
   ...
   */
  var probability = 0, shift, occurrences, fieldId;
    ...
    for (term in uniqueTerms[fieldId]) {
      if (uniqueTerms[fieldId].hasOwnProperty(term)) {
        ...
        if (this.tagClouds.hasOwnProperty(fieldId)) {
          index = this.tagClouds[fieldId].index(term);
        } else if (this.categories[fieldId].hasOwnProperty(fieldId)) {
          index = this.categories[fieldId].index(term);
        }
