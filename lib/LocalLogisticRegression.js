          ...
          if (this.tagClouds.hasOwnProperty(fieldId)) {
            ...
            if (this.fieldCodings.hasOwnProperty(fieldId) &&
              ...
              for (coeffIndex = 0;
                ...
                norm2 = Math.pow(contribution * occurrences, 2);
              }
            }
    ...
    if (this.balanceFields) {
      for (field in newInputData) {
        if (newInputData.hasOwnProperty(field) &&
          ...
          newInputData[field] = (newInputData[field] - mean) / stddev;
        }
      }
    }
    // if normalize is true, input data should be normalized:
    if (this.lrNormalize) {
      for (field in mewInputData) {
        if (newInputData.hasOwnProperty(field) &&
            this.fields[field].optype === 'numeric') {
