...
Execution.prototype.create = function (script, args, retry, cb) {
  /**
   ...
   *
   * @param {string|object} script Script id (or object)
  ...
  var options,
    message = 'Failed to create the execution. First parameter must be' +
              ' a script.',
    resourceId = utils.getResource(script),
    self = this;

  options = utils.optionalCUParams(arguments, message);

  self.parent.create.call(self, 'execution', ['script'], message,
                          resourceId.resource,
                          options.args, options.retry, options.cb);
};

Execution.prototype.list = function (query, cb) {
  this.parent.list.call(this, 'execution', query, cb);
};
