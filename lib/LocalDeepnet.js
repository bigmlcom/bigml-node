...
function expandTerms(termsList, inputTerms) {
  ...
  for (var index = 0; index < termsList.length; index++) {
    termOccurrences[index] = 0.0;
  }
  for (var termInfo in inputTerms) {
    if(inputTerms.hasOwnProperty(termInfo)) {
      var index = termsList.indexOf(termInfo);
      if (index !== -1) {
        termOccurrences[index] += 1;
      }
    }
  }
...
function LocalDeepnet(resource, connection) {
  ...
  // Loads the deepnet from the deepet id or from a finished object
  if (NODEJS && ((typeof resource) === 'string' ||
    ...
    if ((typeof self.resourceId) === 'undefined') {
      ...
      utils.tryStoredFile(resource, "deepnet", fillStructure);
    } else {
      ...
      if (filename != null) {
        return new LocalDeepnet(filename);
      }
...
LocalDeepnet.prototype.toPrediction = function (yOut) {
  ...
  for (index = 0, len = this.classNames.length; index < len; index++) {
    ...
    distribution.push({category: category,
                       probability: utils.truncate(prediction[index], 5)});
