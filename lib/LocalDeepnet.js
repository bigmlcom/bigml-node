...
function LocalDeepnet(resource, connection) {
  ...
  fillStructure = function (error, resource) {
    ...
    var status, fields, field, fieldInfo, index, deepnetInfo,
      ...
      fieldId, realId, key, contributions, network;
...
LocalDeepnet.prototype.fillArray = function(inputData, uniqueTerms) {
  ...
  var columns = [], termsOccurrences, fieldId;
  for (fieldId in this.inputFields) {
...
LocalDeepnet.prototype.deepnetPredict = function (inputData) {
  ...
  var uniqueTerms, probabilities = [], total = 0, len, probability,
    category, coefficients, order, index, probabilityInfo, inputArray;
  ...
  yOut = net.propagate(inputArray, this.layers);
  if (this.regression) {
  ...
  distribution = [];
  for (index = 0; index < this.classNames.length; index++) {
    category = this.classNames[index];
    distribution.push({category: category, probability: yOut[0][index]});
  }

  return {
    prediction: this.classNames[prediction[0]],
