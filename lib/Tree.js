...
function Tree(tree, fields, objectiveField, rootDistribution) {
  ...
  var child, summary, weighted_summary, children = [];
  if (tree.children) {
  ...
  if (tree.distribution) {
    ...
    if (tree['weighted_objective_summary']) {
      weighted_summary = tree['weighted_objective_summary'];
      this.weighted_distribution = weighted_summary.categories;
      this.weighted = true;
    }
  } else {
...
Tree.prototype.predict = function (inputData, path, missingStrategy, median,
   ...
   * @param {boolean} missingFound This flag is set on when a split is found
   *                  depending on a text or item field which is missing.
   */
...
Tree.prototype.predictProportional = function (inputData, path,
  ...
  var child, numChild, predicate, finalDistribution = {}, counter, key,
    distributionDicts = {}, newRule, cDistribution, fieldType, population = 0,
    childPredictions, dist;
  if (!path) {
    path = [];
  }
  if (this.children.length === 0) {
    dist = this.weighted ? this.weighted_distribution : this.distribution;

    for (counter = 0; counter < dist.length; counter++) {
      key = dist[counter][0];
      distributionDicts[key] = dist[counter][1];
    }
    return {distribution: mergeDistributions({}, distributionDicts),
            lastNode: this, population: this.count};
  }
  ...
  if (oneBranch(this.children, inputData) ||
    ...
    return {distribution: finalDistribution,
            lastNode: this, population: population};
  }
};
