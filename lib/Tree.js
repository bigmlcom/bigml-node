...
function regressionError(distVariance, population, zeta) {
  ...
  if (population > 0) {
    var error, j$ = (NODEJS) ? jStat.j$ : jStat,
      chiDistr = j$.chisquare(population),
      ppf = chiDistr.inv(1 - j$.erf(zeta / Math.sqrt(2)));
    if (ppf !== 0) {
...
Tree.prototype.predict = function (inputData, path, missingStrategy) {
   ...
   *
   */
  var child, predicate, finalDistribution, distribution = [],
  ...
  if (missingStrategy === constants.PROPORTIONAL) {
    finalDistribution = this.predictProportional(inputData, path);
    if (this.regression) {
