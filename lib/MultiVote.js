...
function softmax(predictions):
  ...
  var total = 0.0, normalized = {}, keys = Object.keys(predictions),
    len = keys.length, classPrediction, category, result = {};
  for (index = 0; index < len; index++) {
    category = keys[index];
 ...
 */
function MultiVote(predictions, boosting) {
  this.predictions = [];
  this.boosting = boosting
  var i, len;
  if (typeof boosting !== 'undefined') {
    this.boosting = false;
  }
...
MultiVote.prototype.combine = function (method, options) {
  ...
  var keys = WEIGHT_KEYS[method], predictions = this, index = 0, prediction,
    len = self.predictions.length;
  ...
  if (this.boosting) {
    for (index = 0; index < len; index++) {
      prediction = this.predictions[index];
      if (typeof prediction[WEIGHT_LABELS[BOOSTING]] === 'undefined') {
        prediction[WEIGHT_LABELS[BOOSTING]] = 0;
      }
    }
    if (this.is_regression) {
      return weightedSum(this.predictions, WEIGHT_LABELS[BOOSTING]);
    } else {
