...
MultiVote.prototype.combine = function (method, options) {
  ...
  var weight, keys, predictions = this, index = 0, prediction, len;
  ...
  weight = (method > 0 ? WEIGHT_LABELS[method]:
    WEIGHT_LABELS[WEIGHT_LABELS.length + method]);
  // there must be at least one prediction to be combined
  if (this.predictions.length === 0) {
    throw new Error('No predictions to be combined.');
  }

  keys = (method > 0 ? WEIGHT_KEYS[method] :
    WEIGHT_KEYS[WEIGHT_KEYS.length + method]);
  ...
  if (this.boosting) {
    ...
    if (this.isRegression()) {
      return weightedSum(this.predictions, weight);
    } else {
