...
var WEIGHT_LABELS = [undefined, 'confidence', 'probability'];
var PLURALITY = 0;
var CONFIDENCE = 1;
var PROBABILITY = 2;
  ...
  for (index = 0, len = predictions.length; index < len; index++) {
    ...
    for (kindex = 0; kindex < keys.length; kindex++) {
      ...
      if (!prediction.hasOwnProperty(key)) {
        throw "Not enough data to use the selected prediction method.";
      }
    }
  }
}

function wsConfidence(prediction, distribution, n, z) {
  /**
  ...
  z = ((typeof z) !== 'undefined') ? parseFloat(z) : 1.96;
  var norm, z2, n2, wsSqrt, key, p = distribution[prediction];
  if (p < 0) {
    throw "The distribution weight must be a positive value";
  }
  if (n < 1) {
    throw "The total of instances in the distribution must be a positive integer";
  }
  norm = 0.0;
...
MultiVote.prototype.combineCategorical = function (weightLabel) {
   ...
   *        undefined:        plurality (1 vote per prediction)
  ...
  for (index = 0, len = this.predictions.length; index < len; index++) {
    ...
    if (typeof weightLabel !== 'undefined') {
      if (WEIGHT_LABELS.indexOf(weightLabel) === -1) {
        throw "Wrong weightLabel value.";
      }
      if (!prediction.hasOwnProperty(weightLabel)) {
        throw "Not enough data to use the selected prediction method. Try creating your model anew.";
      }
  ...
  combinedConfidence = wsConfidence(prediction, distribution, count);
  ...
  if (typeof weightLabel !== 'undefined') {
    for (index = 0, len = predictions.length; index < len; index++) {
      ...
      if ((typeof weightLabel !== 'string') ||
        ...
        throw "Not enough data to use the selected prediction method. Lacks " +
          weightLabel + " information";
      }
    }
  }
  for (index = 0, len = predictions.length; index < len; index++) {
...
MultiVote.prototype.probabilityWeight = function () {
  ...
  for (index = 0, len = this.predictions.length; index < len; index++) {
    ...
    if (!prediction.hasOwnProperty('distribution') ||
      ...
      throw "Probability weighting is not available because distribution" +
    ...
    if (total < 1 || (typeof total !== "number" ||
      ...
      throw "Probability weighting is not available because distribution" +
        "seems to have " + total + " as number of instances in a node";
    }
    order = prediction.order;
...
MultiVote.prototype.combine = function (method) {
  /**
  ...
  if (this.predictions.length === 0) {
    throw "No predictions to be combined.";
  }
...
MultiVote.prototype.normalizeError = function (topRange) {
  ...
  var  error, index, len, errorRange, prediction, delta, maxError = -1,
    minError = -1, normalizeFactor = 0;
  for (index = 0, len = this.predictions.length; index < len; index++) {
    prediction = this.predictions[index];
