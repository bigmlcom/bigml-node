...
exports = {
  ...
  cast: function (inputData, fields, roundInt) {
    /**
     ...
     * @param {boolean} checks whether an input data field is integer and
     *                  rounds the input value if needed. Defaults to true
     */
    ...
    for (field in inputData) {
      if (inputData.hasOwnProperty(field)) {
        ...
        if ((integerDataTypes.indexOf(fields[field].datatype) >= 0) &&
            (inputData[field] % 1 !== 0) &&
            (typeof roundInt === 'undefined' || roundInt)) {
 ...
 sortCategory: function(categoryA, categoryB) {
  /**
   * Sorts aphabetically by category name
   *
   * @param {object} categoryA Category in a prediction node object
   * @param {object} categoryB Category in a prediction node object
   */
