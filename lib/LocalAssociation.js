...
function LocalAssociation(resource, connection) {
  ...
  // Loads the association from the association id or from a finished object
  if ((NODEJS && ((typeof resource) === 'string')) ||
    ...
    if ((typeof self.resourceId) === 'undefined') {
      ...
      // if a resource id has been found, then load the association
      ...
      // try to load the association info from the storage directory
      if (typeof connection !== 'undefined' &&
        ...
        try {
          ...
          if (fs.existsSync(filename)) {
            return new LocalAssociation(filename);
          }
        } catch (err) {}
      }
...
LocalAssociation.prototype.retrieveAssociationSet = function (
  ...
  for (index = 0; index < len; index++) {
    ...
    if (cosine > 0) {
      cosine = cosine / (Math.sqrt(itemsIndexes.length) *
  ...
  for (index = 0; index < len; index++) {
    ...
    finalPredictions[index][1]["item"] = this.items[
      parseInt(rhs[0], 10)].toJSON(INTERNAL_KEYS);
