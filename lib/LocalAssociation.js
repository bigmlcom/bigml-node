...
LocalAssociation.prototype.associationSet = function (
  ...
  function createLocalAssociationSet(error, inputData, k, scoreBy, cb) {
    /**
...
LocalAssociation.prototype.retrieveAssociationSet = function (
  ...
  for (index = 0; index < len; index++) {
    ...
    finalPredictions[index][1]["item"] = this.items[parseInt(rhs[0], 10)].toJSON();
    // adapting to association set format
    for (index2 = 0; index2 < INTERNAL_KEYS; index2++) {
      delete finalPredictions[index][1].item.INTERNAL_KEYS[index2];
    }
