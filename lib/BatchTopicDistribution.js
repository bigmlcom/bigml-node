...
function BatchTopicDistribution(connection) {
  Resource.call(this, connection);
}

BatchTopicDistribution.prototype = new Resource();

BatchTopicDistribution.prototype.parent = Resource.prototype;

BatchTopicDistribution.prototype.create = function (topicModel,
                                                    dataset, args, retry, cb) {
  /**
   * Creates a batch topic distribution and builds customized error and
   * resource info
   *
   ...
   *
   * @param {string|object} topicModel Topic Model id (or resource)
  ...
  var self = this, options, resources,
    origins = [constants.TOPIC_MODEL],
    message = 'Failed to create the batch topic distribution. First' +
    ' parameter must be a topic model id.',
    resourceId,
    datasetId,
    sendRequest,
    reqOptions = utils.createRequest(constants.BATCH_TOPIC_DISTRIBUTION);
...
BatchTopicDistribution.prototype.list = function (query, cb) {
  this.parent.list.call(this, constants.BATCH_TOPIC_DISTRIBUTION,
                        query, cb);
};

BatchTopicDistribution.prototype.download = function (resource, filename, cb) {
  /**
   * Downloads the batch topic distribution output file.
   *
   * @param {object|string} resource Batch topic distribution resource or id
...
BatchTopicDistribution.prototype.createAndWait = function (
  topicModel, dataset, args, retry, cb) {
  var self = this;
  ...
  self.create(topicModel, dataset, args, retry,
    function(error, data) {
...
module.exports = BatchTopicDistribution;
