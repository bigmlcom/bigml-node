...
function LocalLinearRegression(resource, connection) {
  ...
  this.bias = undefined;
  this.xtx = [];
  this.mean_squared_error = undefined;
  this.number_of_parameters = undefined;
  this.number_of_samples = undefined;
  ...
  fillStructure = function (error, resource) {
    ...
    if ((typeof resource['linear_regression']) !== 'undefined') {
      if (status.code === constants.FINISHED) {
        ...
        self.weightField = linearRegressionInfo['weight_field'];
        stats = linearRegressionInfo["stats"];
        self.xtx = stats.xtx;
        self.meanSquaredError = stats["mean_squared_error"];
        self.numberOfParameters = stats["number_of_parameters"];
  ...
  if (NODEJS && ((typeof resource) === 'string' ||
    ...
    if ((typeof self.resourceId) === 'undefined') {
      ...
      utils.tryStoredFile(resource, "linear regression", fillStructure);
    } else {
      // if a resource id has been found, then load the model
      // if no connection info is provided, create one default connection with
      // local storage
      filename = utils.getStoredModelFile(this.resourceId, connection);
      if (filename != null) {
        return new LocalLinearRegression(filename);
      }
      linearRegression = new LinearRegression(connection);
      linearRegression.get(this.resourceId.resource, true,
...
LocalLinearRegression.prototype.linearPredict = function (inputData) {
  ...
  prediction = {prediction: utils.dot(flatten(this.coefficients),
                                      inputArray),
                confidenceBounds: this.confidenceBounds(compactInputArray)};
  return prediction;
};
...
LocalLinearRegression.prototype.confidenceBounds = function (inputArray) {
   ...
   */
  var dof = this.numberOfSamples - this.numberOfParameters,
    tDist = jStat.studentt(dof),
    // alpha 0.05
    tCrit = tDist.inv(0.975),
    invXtX = jStat.inv(this.xtx),
    product = [], index, len, confidenceInterval, predictionInterval,
    index2, len2, total;

  for (index = 0, len = invXtX.length; index < len; index++) {
    total = 0;
    for (index2 = 0, len2 = invXtX[index].length;
         index2 < len2; index2++) {
      total += inputArray[index2] * invXtX[index][index2];
    }
    product.push(total);
  }

  product = jStat.dot(product, inputArray);

  confidenceInterval = tCrit * Math.sqrt(this.meanSquaredError * product);
  predictionInterval = tCrit * Math.sqrt(this.meanSquaredError * (1 + product));
