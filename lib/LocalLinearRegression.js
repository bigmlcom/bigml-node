...
LocalLinearRegression.prototype.linearPredict = function (inputData) {
  ...
  var uniqueTerms, index, inputInfo, inputArray, prediction;
  ...
  index = 0;
  inputArray = this.expandInput(inputData, uniqueTerms);
  prediction = mathOps.dot([flatten(this.coefficients)], [inputArray])[0][0];
...
LocalLinearRegression.prototype.expandInput = function (inputData, uniqueTerms) {
  ...
  for (index = 0; index < len; index++) {
    ...
    if (field.summary["missing_count"] > 0 ||
      ...
      newInputs.push(parseInt(missing));
    }
    if (optype == CATEGORICAL) {
      newInputs = this.categoricalEncoding(newInputs, fieldId);
    }
