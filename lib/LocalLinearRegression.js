  ...
  fillStructure = function (error, resource) {
    ...
    if ((typeof resource['linear_regression']) !== 'undefined') {
      if (status.code === constants.FINISHED) {
        ...
        self.invXtX = jStat.inv(self.xtx);
...
LocalLinearRegression.prototype.confidenceBounds = function (inputArray) {
  ...
  for (index = 0, len = this.invXtX.length; index < len; index++) {
    total = 0;
    for (index2 = 0, len2 = this.invXtX[index].length;
         index2 < len2; index2++) {
      total += inputArray[index2] * this.invXtX[index][index2];
    }
    product.push(total);
  }
