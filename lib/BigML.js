...
function BigML(username, apiKey, devMode, context) {
  ...
  if (typeof context !== 'undefined' && context.storage !== 'undefined') {
    try {
...
BigML.prototype.request = function (options, retry, cb) {
  ...
  var reqOptions, self = this, filename, storageFile,
    ...
    if (body) {
      ...
      try {
        ...
        if (typeof self.storage !== 'undefined' &&
            self.storage !== null && options.method !== "DELETE") {
          filename = path.join(self.storage, json.resource.replace("/", "_"));
          storageFile = fs.createWriteStream(filename, {'flags': 'w'});
          storageFile.write(body);
          storageFile.end();
          storageFile.close();
        }
