...
function BigML(username, apiKey, devMode, context) {
  ...
  if (typeof context !== 'undefined') {
    if (context.hasOwnProperty("protocol")) {
      protocol = context.protocol;
    } else {
      protocol = constants.BIGML_PROTOCOL;
    }
    if (context.hasOwnProperty("domain")) {
      domain = context.domain;
    } else {
      domain = constants.BIGML_DOMAIN;
    }
    this.url = protocol + "://" + domain;
    if (devMode) {
      this.url = this.url + '/dev/andromeda';
    } else {
      this.url = this.url + '/andromeda';
    }
  } else {
    if (devMode) {
