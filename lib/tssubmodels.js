...
function seasonContribution(s, m, h) {
  /**
   * Computing the contribution of each season component
   *
   */
  var index = 0;
  if (m > 0 && Object.prototype.toString.call(s) === '[object Array]' &&
     s.length === m) {
...
var OPERATORS = {"A": function(x, s, m, h) {
                        return x + seasonContribution(s, m, h);},
                 "M": function(x, y) {
                        return x * seasonContribution(s, m, h);},
...
function NForecast(submodel, horizon, seasonality) {
  ...
  var points = [], h, finalState = submodel["final_state"] || {},
    l = finalState.l || 0, s = finalState.s || 0, m = submodel.period || 0;

  for (h = 0; h < horizon; h++) {
    points.push(OPERATORS[seasonality](l, s, m, h));
  }
  return points;
}
...
function AForecast(submodel, horizon, seasonality) {
  ...
  for (h = 0; h < horizon; h++) {
    points.push(OPERATORS[seasonality](l + b * (h + 1), s, m, h));
  }
  return points;
}
...
function AdForecast(submodel, horizon, seasonality) {
  ...
  var points = [], h, finalState = submodel["final_state"] || {},
    l = finalState.l || 0, b = finalState.b || 0, s = finalState.s || 0,
    phi = finalState.phi || 0, phi_h = phi, m = submodel.period || 0;
  for (h = 0; h < horizon; h++) {
    points.push(OPERATORS[seasonality](l + b * phi_h, s, m, h));
...
function MForecast(submodel, horizon, seasonality) {
  ...
  var points = [], h, finalState = submodel["final_state"] || {},
    ...
    m = submodel.period || 0;
  for (h = 0; h < horizon; h++) {
    points.push(OPERATORS[seasonality](l + Math.pow(b, h + 1), s, m, h));
  }
  return points;
}
...
function MdForecast(submodel, horizon, seasonality) {
  ...
  var points = [], h, finalState = submodel["final_state"] || {},
    ...
    phi = finalState.phi || 0, phi_h = phi, m = submodel.period || 0;
  for (h = 0; h < horizon; h++) {
    points.push(OPERATORS[seasonality](l * Math.pow(b, phi_h), s, m, h));
    phi_h = phi_h + Math.pow(phi, h + 1);
  }
  return points;
}
