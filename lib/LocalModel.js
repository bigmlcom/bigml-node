...
LocalModel.prototype.predict = function (inputData, missingStrategy, cb) {
  /**
  ...
  if (arguments.length < 3 && (typeof missingStrategy === 'function')) {
    // donwgrading gently to old syntax with no missingStrategy
    cb = missingStrategy;
  ...
  function createLocalPrediction(error, data) {
    ...
    prediction = self.tree.predict(data, null, missingStrategy);
  ...
  if (this.ready) {
    ...
    this.on('ready', function (self) {
            return self.predict(inputData, missingStrategy, cb);
          });
    return;
  }
};
