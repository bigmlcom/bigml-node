...
var Prediction = require(PATH + 'Prediction');
  ...
  fillStructure = function (error, resource) {
    ...
    if ((typeof resource.model) !== 'undefined') {
      if (status.code === constants.FINISHED) {
        ...
        self.objectiveField = self.tree.objectiveField;
        self.regression = ((
          !self.boosting &&
...
LocalModel.prototype.boostingPrediction = function (prediction,
  ...
  prediction.weight = self.boosting.weight;
  if (typeof self.boosting.objective_class !== 'undefined') {
    prediction.objClass = self.boosting.objective_class;
  }
  return prediction;
}
...
LocalModel.prototype.predict = function (inputData, missingStrategy,
  ...
  function createLocalPrediction(error, data) {
    ...
    if (self.boosting) {
      ...
      prediction = self.boostingPrediction(prediction);
    } else {
...
LocalModel.prototype.validateInput = function (inputData,
  ...
  if (this.ready) {
    ...
    try {
      newInputData = utils.cast(newInputData, this.tree.fields);
    } catch (err) {
      if (cb) {
        return cb(err, null);
      }
