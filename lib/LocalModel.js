...
LocalModel.prototype.predict = function (inputData, missingStrategy,
  ...
  function createLocalPrediction(error, data) {
    ...
    prediction = self.tree.predict(data.inputData, null, missingStrategy, median,
                                   true, []);
    if (addUnusedFields) {
...
LocalModel.prototype.validateInput = function (inputData,
  ...
  if (this.ready) {
    ...
    if (cb) {
      return cb(null, {inputData: inputData, unusedFields : unusedFields});
    }
    return {inputData: inputData, unusedFields: unusedFields};
  }
  this.on('ready', function (self) {
