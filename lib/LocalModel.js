...
function LocalModel(resource, connection, fields) {
  ...
  // Loads the model from the model id or from a finished object
  if (NODEJS && ((typeof resource) === 'string' ||
    ...
    if ((typeof self.resourceId) === 'undefined') {
      ...
      model = utils.tryStoredFile(resource, "model");
      try {
        this.resourceId = utils.getResource(model);
        resource = model;
      } catch (e) {
        this.resourceId = undefined;
        throw new Error("No valid model information found in " +
                        resource);
      }
...
LocalModel.prototype.predictConfidence = function (inputData, missingStrategy,
  ...
  function syncPredictConfidence(error, prediction) {
    ...
    if (self.isRegression || self.boosting) {
