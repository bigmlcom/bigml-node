...
Test.prototype = new Resource();

Test.prototype.parent = Resource.prototype;

Test.prototype.create = function (dataset, args, retry, cb) {
  /**
  ...
  var options,
    message = 'Failed to create the test. First parameter must be' +
              ' a dataset.',
  ...
  self.parent.create.call(self, 'test', ['dataset'], message,
                          resourceId.resource,
                          options.args, options.retry, options.cb);
};

Test.prototype.list = function (query, cb) {
  this.parent.list.call(this, 'test', query, cb);
};
