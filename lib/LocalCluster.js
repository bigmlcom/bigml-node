...
var LocalCentroid = require('./LocalCentroid');
...
function getUniqueTerms(terms, termForms, tagCloud) {
  ...
  for (i = 0; i < termsLength; i++) {
    ...
    if ((termsSet.indexOf(term) < 0) && tagTerms.indexOf(term) > -1) {
      termsSet.push(term);
    } else if ((termsSet.indexOf(term) < 0) && extendForms.hasOwnProperty(term)) {
...
LocalCluster.prototype.computeNearest = function (inputData) {
  ...
  var uniqueTerms = {}, terms, caseSensitive, tokenMode, inputDataField,
    fieldId, nearest, distance, i, clustersLength, centroid;
  for (fieldId in this.tagClouds) {
    if (inputData.hasOwnProperty(fieldId)) {
      caseSensitive = this.termAnalysis[fieldId].caseSensitive;
  ...
  for (i = 0; i < clustersLength; i++) {
    ...
    distance = centroid.distance(inputData, uniqueTerms, this.scales,
                                 nearest.distance);
...
LocalCluster.prototype.validateInput = function (inputData, cb) {
  ...
  if (this.ready) {
    ...
    try {
      inputData = cast(newInputData, this.fields);
    } catch (err) {
      if (cb) {
        return cb(err, null);
      }
