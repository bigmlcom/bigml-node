...
exports = {
  ...
  npC: function (arrayA, arrayC) {
    var newArray = [];
    if ([null, []].indexOf(arrayA)) {
      return arrayC
    }
    newArray = JSON.stringify(JSON.parse(arrayA));
    newArray[0].concat(arrayC);
  },
  ...
  standardize: function(vector, stats) {
    var newVec = [], mn = stats[0], stdev = stats[1];
  ...
  preprocess: function (columns, specs) {
    var outdata, outarray;
    if (spec.type == c.NUMERIC) {
      column = exports.npAsArray(column, dtypeFn='float');
    }
