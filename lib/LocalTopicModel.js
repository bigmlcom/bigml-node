...
function LocalTopicModel(resource, connection) {
  ...
  if ((typeof this.resourceId) === 'undefined') {
    throw new Error('Cannot build a TopicModel from this resource: ' +
                    resource);
  }
  ...
  if ((NODEJS && ((typeof resource) === 'string')) ||
    ...
    if ((typeof self.resourceId) === 'undefined') {
      ...
      try {
        ...
        throw new Error('Cannot build a Topic Model from this resource: ' +
                        resource);
      }
    } else {
      // if a resource id has been found, then load the topic model
      ...
      if (typeof connection !== 'undefined' &&
        ...
        try {
          ...
          if (fs.existsSync(filename)) {
            return new LocalTopicModel(filename);
          }
        } catch (err) {}
      }
