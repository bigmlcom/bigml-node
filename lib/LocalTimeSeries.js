    ...
    if ((typeof self.resourceId) === 'undefined') {
      ...
      try {
        fs.readFile(resource, 'utf8', function (err, data) {
          if (err) {
            throw new Error('Failed to read local time series file: ' + resource);
          }
          try {
            resource = JSON.parse(data);
            fillStructure(null, resource);
          } catch (jsonErr) {
            throw new Error('Failed to parse the JSON time series in: ' + resource);
          }
        });
      } catch (errf) {
        // if no file is read, throw error reading file
        throw new Error('Cannot build a Time Series from this resource: ' +
                        resource);
      }
    } else {
      // if a resource id has been found, then load the time series
      // if no connection info is provided, create one default connection with
      // local storage
      if (typeof connection === 'undefined') {
        connection = new BigML(undefined, undefined, false,
                               {storage: './storage'});
      }
      // try to load the time series info from the storage directory
      if (typeof connection !== 'undefined' &&
          typeof connection.storage !== 'undefined') {
        try {
          filename = path.join(connection.storage,
                               this.resourceId.resource.replace("/", "_"));
          if (fs.existsSync(filename)) {
            return new LocalTimeSeries(filename);
          }
        } catch (err) {}
      }
