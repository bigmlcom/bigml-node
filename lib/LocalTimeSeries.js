...
function computeForecasts(submodels, horizon) {
  ...
  for (index = 0; index < len; index++) {
    ...
    if (name.indexOf(",") > -1) {
      ...
      forecasts.push({submodel: name,
        point_forecast: SUBMODELS[trend](
          submodel, horizon, seasonality)});
    } else {
      forecasts.push({submodel: name,
        point_forecast: SUBMODELS[name](submodel, horizon)});
    }
  }
  return forecasts;
}
...
function filterSubmodels(submodels, filterInfo) {
  ...
  if (indices.length == 0 && names.length == 0) {
    fieldSubmodels = submodels.slice(0, submodels.length);
  }
...
function LocalTimeSeries(resource, connection) {
  ...
  var model, self, fillStructure, objectiveField, timeSeries, timeSeriesInfo;

  this.resourceId = undefined;
  this.inputFields = undefined;
  this.objective_fields = [];
  this.all_numeric_objectives = false;
  this.period = 1;
  this.submodels = {};
  this.error = undefined;
  this.damped_trend = undefined;
  this.seasonality = undefined;
  this.trend = undefined;
  this.time_range = {};
  this.field_parameters = {};
  ...
  fillStructure = function (error, resource) {
    ...
    if ((typeof resource['time_series']) !== 'undefined') {
      if (status.code === constants.FINISHED) {
        ...
        self.fields = fields;
        self.all_numeric_objectives = timeSeriesInfo['all_numeric_objectives'];
        self.submodels = timeSeriesInfo.submodels || {};
    ...
    if ((typeof self.resourceId) === 'undefined') {
      ...
      try {
        fs.readFile(resource, 'utf8', function (err, data) {
          if (err) {
            throw new Error('Failed to read local time-series file: ' +
                            resource);
          }
          try {
            ...
            throw new Error('Failed to parse the JSON time-series' +
                            ' in: ' + resource);
          }
        });
      } catch (errf) {
        // if no file is read, throw error reading file
        throw new Error('Cannot build a TimeSeries from this ' +
                        'resource: ' +
                        resource);
      }
    } else {
      // if a resource id has been found, then load the logistic regression
      timeSeries = new TimeSeries(connection);
      timeSeries.get(this.resourceId.resource, true,
...
LocalTimeSeries.prototype.tsForecast = function (inputData) {
  ...
  for (fieldId in inputData) {
    if (inputData.hasOwnProperty(fieldId)) {
      ...
      filterInfo = fieldInput.submodels || DEFAULT_SUBMODEL;
      filteredSubmodels[fieldId] = filterSubmodels(this.submodels[fieldId],
        filterInfo);
    }
  }
