...
function LocalEnsemble(ensembleOrModels, connection, maxModels) {
  ...
  var i, fillEnsembleInfo, fillModelsInfo, fillSeparateModelsInfo, fill,
    addModel, self, ensemble, addBoosting, resource, filename;
  if (!maxModels) {
    ...
    if ((typeof self.resourceId) === 'undefined') {
      ...
      try {
        fs.readFile(resource, 'utf8', function (err, data) {
          if (err) {
            throw new Error('Failed to read local ensemble file: ' + resource);
          }
          try {
            resource = JSON.parse(data);
            fillStructure(null, resource);
          } catch (jsonErr) {
            throw new Error('Failed to parse the JSON ensemble in: ' + resource);
          }
        });
      } catch (errf) {
      ...
      // if a resource id has been found, then load the ensemble
      ...
      if (typeof connection !== 'undefined' &&
        ...
        try {
          ...
          if (fs.existsSync(filename)) {
            return new LocalEnsemble(filename);
          }
        } catch (err) {}
      }
