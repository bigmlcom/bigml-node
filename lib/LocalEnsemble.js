...
function LocalEnsemble(ensembleOrModels, connection, maxModels) {
   ...
   *        in predictions
   */
  var i, fillEnsembleInfo, fillModelsInfo, fillSeparateModelsInfo,
    self, ensemble;
  if (!maxModels) {
  ...
  fillSeparateModelsInfo = function (self) {
    ...
    for (i = 0; i < self.modelIds.length; i++) {
      model.get(self.modelIds[i], true, 'limit=-1', function (error, resource) {
...
LocalEnsemble.prototype.predict = function (inputData, method, cb) {
  ...
  function issuePrediction (err, prediction) {
    if (err) {
      return cb(err, null);
    }
    ...
    for (index = 0; index < len; index++) {
      ...
      for (splitIndex = 0; splitIndex < splitLen; splitIndex++) {
        ...
        if (cb) {
          ...
          predictions.push(model.predict(inputData));
        }
      }
    }
    if (!cb) {
