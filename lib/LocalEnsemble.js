...
function LocalEnsemble(ensembleOrModels, connection, maxModels) {
  ...
  var i, fillEnsembleInfo, fillModelsInfo, fillSeparateModelsInfo, fill,
    addModel, self = this, ensemble, resource, filename,
    localModel;
  if (!maxModels) {
  ...
  fillModelsInfo = function (self) {
    ...
    function createSplit(error, resources) {
      ...
      for (i = 0; i < self.modelsSplits.length; i++) {
        for (j = 0; j < self.modelsSplits[i].length; j++) {
         self.checkModelsReady(self.modelsSplits[i][j]);
        }
      }
    }
  ...
  fillSeparateModelsInfo = function (localEnsemble) {
    ...
    model = new Model(localEnsemble.connection);
    for (i = 0; i < localEnsemble.modelIds.length; i++) {
      if (typeof localEnsemble.connection.storage !== 'undefined') {
        modelObject = utils.getResource({resource: localEnsemble.modelIds[i]});
        filename = utils.getStoredModelFile(modelObject.resource,
                                            localEnsemble.connection);
        if (filename != null) {
          addModel(null, filename);
        } else  {
          model.get(localEnsemble.modelIds[i], true, constants.ONLY_MODEL, addModel);
        }
      } else {
        model.get(localEnsemble.modelIds[i], true, constants.ONLY_MODEL, addModel);
      }
  ...
  if (Object.prototype.toString.call(ensembleOrModels) !== '[object Array]') {
    ...
    if ((typeof this.resourceId) === 'undefined' &&
      ...
      resource = utils.tryStoredFile(ensembleOrModels, "ensemble");
      try {
        this.resourceId = utils.getResource(resource);
        ensembleOrModels = resource;
      } catch (e) {
        this.resourceId = undefined;
        throw new Error("No valid ensemble information found in " +
                        ensembleOrModels);
      }
    }
  ...
  if (Object.prototype.toString.call(ensembleOrModels) === '[object Array]') {
    ...
    if (filename != null) {
      ...
      try {
        ...
        this.resourceId = undefined;
        throw new Error("Loooking for the local copy of the ensemble" +
                        "No valid ensemble information found in " +
                        filename);
      }
