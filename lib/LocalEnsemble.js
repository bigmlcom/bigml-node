...
LocalEnsemble.prototype.predictOperatingKind = function predictOperatingKind (
    ...
    if (this.ready) {
      missingStrategy = missingStrategy || constants.LAST_PREDICTION;
      kind = operatingKind.toLowerCase();
...
LocalEnsemble.prototype.predict = function (inputData, method, options, cb) {
   ...
   *          operatingPoint: map,
   *          operatingKind: {'confidence'|'probability'},
   *          threshold: integer,
   *          category: string,
