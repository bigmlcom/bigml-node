...
function LocalEnsemble(ensembleOrModels, connection, maxModels) {
  ...
  if (Object.prototype.toString.call(ensembleOrModels) === '[object Array]') {
    ...
    if (typeof this.boosting === "undefined") {
      if (this.modelsSplit[0][0].boosted_ensemble) {
        this.boosting = this.modelsSplits[0][0].boosting;
      }
      this.fields = this.modelsSplits[0][0].fields;
      if (this.boosting) {
        this.objectiveField = this.boosting.objective_field ;
      } else {
        this.objectiveField = this.modelsSplits[0][0].objectiveField;
      }
      this.isRegression = this.fields[this.objectiveField].optype == 'numeric';
    }
