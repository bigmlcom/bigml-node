...
var PATH = (NODEJS) ? "./" : (
  (typeof window !== 'undefined') && (typeof window.PATH !== 'undefined')) ?
  window.PATH : "";
...
var DEFAULT_MAX_MODELS = 200;
var DEFAULT_MAX_PARALLEL_DOWNLOADS = 10;
var BOOSTING = 1;
...
function LocalEnsemble(ensembleOrModels, connection, maxModels,
  ...
  var i, fillEnsembleInfo, fillModelsInfo, fillSeparateModelsInfo, fill,
    ...
    localModel, fillModelsBatch;
  if (!maxModels) {
    maxModels = DEFAULT_MAX_MODELS;
  }
  if (!maxParallelDownloads) {
  ...
  this.downloadSlots = maxParallelDownloads;
  this.batchOffset = 0;
  ...
  fillEnsembleInfo = function (error, resource) {
     ...
     */
    var index, len, index2, len2, categories, batches;

    if (!self.ensembleReady) {
      if (error) {
      ...
      if (self.modelsSplits.length == 0) {
        batches = Math.ceil((self.modelIds.length + 0.0) / self.maxModels);
        if (batches > 0) {
          for (i = 0; i < batches; i++) {
            self.modelsSplits[i] = [];
          }
  ...
  fillModelsInfo = function (self) {
    ...
    function createSplit(error, resources) {
      ...
      var i, localModels = [], localModel, j, k,
        len = resources.resources.length;
      for (k = 0; k < len; k += self.maxParallelDownloads) {
        buffer = resources.resources.slice(k, k + self.maxParallelDownloads);
        for (i = 0; i < buffer.length; i++) {
          modelObject = buffer[i];
        ...
        for (i = 0; i < buffer.length; i++) {
          self.checkModelsReady(localModels[k + i]);
        }
      }
      self.modelsSplits.push(localModels);
    }

    if (self.maxModels != null) {
        query += ';limit=' + self.maxModels;
    }

    if (self.maxModels == null || self.maxModels >= self.modelIds.length) {
      self.maxModels = self.modelIds.length;
      model.list(query, createSplit);
    } else {
      for (i = 0; i < self.modelIds.length; i += self.maxModels) {
        model.list(query + ';offset=' + i, createSplit);
      }
    }
  };

  addModel = function (error, resource, batch) {
    /**
    ...
    self.modelsSplits[batch].push(localModel);
    // needed for GC to work
    setTimeout(function() {self.checkModelsReady(localModel);}, 0);
  };

  fillModelsBatch = function (localEnsemble) {
    var offset, totalModels, batch;
    offset = localEnsemble.batchOffset;
    totalModels = localEnsemble.modelIds.length;
    for (i = offset; i < totalModels; i++) {
      if (localEnsemble.downloadSlots == 0) {
        break;
      }
      localEnsemble.batchOffset++;
      localEnsemble.downloadSlots--;
      batch = - 1 + Math.ceil((localEnsemble.batchOffset + 0.0) /
                              localEnsemble.maxModels);
      addModel(null, localEnsemble.modelIds[i], batch);
    }
  };
  ...
  if (Object.prototype.toString.call(ensembleOrModels) === '[object Array]') {
    ...
    if (NODEJS) {
      self.on('ensembleReady', fillModelsBatch);
      self.on('batchReady', fillModelsBatch);
    }
    filename = null;
    ...
    if (filename != null) {
      ...
      try {
        ...
        fillEnsembleInfo(null, resource);
          ...
          storageError = e;
          if (!self.ensembleReady) {
            // the error can happen when reading the ensemble file or
            // after that, with the model files. Only trying to
            // download the ensemble in the first case
            self.resourceId = utils.getResource(resource);
...
LocalEnsemble.prototype.checkModelsReady = function (localModel) {
  ...
  if (localModel.ready) {
    ...
    if (self.modelIds.length === self.modelsReadyCount) {
      ...
      if (NODEJS) {
        if (self.downloadSlots == self.maxParallelDownloads){
          self.emit('batchReady', self);
        }
      }
    }
  } else {
