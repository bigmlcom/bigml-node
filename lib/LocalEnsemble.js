...
LocalEnsemble.prototype.predictVotes = function (
  ...
  if (this.ready) {
    ...
    createPredictionVector = function (prediction) {
      predictionList = [];
      for (kIndex = 0, len = self.classNames.length; kIndex < len; kIndex++) {
        className = self.classNames[kIndex];
        votes = (prediction['prediction'] == className) ? 1 : 0;
        predictionList.push({"prediction": className, "votes": votes});
      }
...
LocalEnsemble.prototype.predictOperating = function (
  ...
  var positiveClass, kind, predictMethod, threshold, issuePrediction,
...
LocalEnsemble.prototype.predict = function (inputData, method, options, cb) {
   ...
   *           missingStrategy: {0|1}, operatingPoint: map}]}}
   *        options Threshold
   *        and category information object, info about fields not used in
   *        the ensemble, strategy for missing values or operating point
   * @param {function} cb Callback
   */

  var index, len, splitIndex, splitLen, predictions, votes, model,
    missingStrategy, data, addUnusedFields, mean, prediction, unused,
    unusedFields, categories = [], operatingPoint,
  ...
  if (this.ready) {
    ...
    if ((typeof options !== 'undefined') &&
        (options.hasOwnProperty('operatingPoint'))) {
