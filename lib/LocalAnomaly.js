    ...
    if ((typeof self.resourceId) === 'undefined') {
      ...
      try {
        fs.readFile(resource, 'utf8', function (err, data) {
          if (err) {
            throw new Error('Failed to read local anomaly file: ' + resource);
          }
          try {
            resource = JSON.parse(data);
            fillStructure(null, resource);
          } catch (jsonErr) {
            throw new Error('Failed to parse the JSON anomaly in: ' + resource);
          }
        });
      } catch (errf) {
        // if no file is read, throw error reading file
        throw new Error('Cannot build an Anomaly from this resource: ' +
                        resource);
      }
    } else {
      // if a resource id has been found, then load the anomaly
      ...
      // try to load the anomaly info from the storage directory
      if (typeof connection !== 'undefined' &&
        ...
        try {
          ...
          if (fs.existsSync(filename)) {
            return new LocalAnomaly(filename);
          }
        } catch (err) {}
      }
