var assert = require('assert'),
  bigml = require('../index'),
  fs = require('fs'),
  credentials = './test/credentials.txt';

try {
  var data = fs.readFileSync(credentials).toString().trim(),
    credentialsArray = data.split(","), username = credentialsArray[0],
    apiKey = credentialsArray[1];
...
} catch(e) {console.log("To use this test, place a 'credentials.txt' file " +
                        "in your test directory with username and API key " +
                        "separated by comma.");};
